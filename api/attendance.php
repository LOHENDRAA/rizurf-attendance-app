...
try {
    ...
    // in a different employee is refused here, before anything else happens.
    ...
    $find->execute([$employee['id'], $today]);
    $existing = $find->fetch();
    ...
    if ($action === 'in') {
        ...
        $insert->execute([$employee['id'], $employee['name'], $today, $now, $mode, $latitude, $longitude, $mode === 'Office' ? OFFICE_QR : null, $status]);
    } else {
    ...
    $records = currentRecords($pdo, $employee['id']);
    $todayStatement = $pdo->prepare('SELECT * FROM attendance_records WHERE employee_id = ? AND attendance_date = ? LIMIT 1');
    $todayStatement->execute([$employee['id'], $today]);
    $todayRecord = $todayStatement->fetch();
