...
import {
  Bell, Briefcase, Calendar, Check, ChevronLeft, ChevronRight, Clock3, Download,
...
function AdminCalendarDay({ date, isSelected, isToday, dayData, filterIntern, statusFilter, holidayName, onSelect }) {
  if (!date) return <span className="admin-calendar-day empty"></span>

  const matchesStatusFilter = !statusFilter || dayData?.status === statusFilter
  const className = [
    'admin-calendar-day',
    isSelected && 'selected',
    isToday && 'is-today',
    holidayName && 'is-holiday',
    filterIntern && statusFilter && dayData && !matchesStatusFilter && 'dimmed',
  ].filter(Boolean).join(' ')

  let badge = null
  if (dayData) {
    if (filterIntern) {
      badge = <span className={`history-dot ${statusDotClass(dayData.status)}`} title={dayData.status}></span>
    } else if (statusFilter === 'Late' && dayData.late > 0) {
      badge = <span className="admin-calendar-count late-count">{dayData.late}</span>
    } else if (statusFilter === 'On time' && dayData.onTime > 0) {
      badge = <span className="admin-calendar-count">{dayData.onTime}</span>
    } else if (!statusFilter) {
      badge = <span className="admin-calendar-count">{dayData.total}</span>
    }
  }
  const showLateDot = !filterIntern && !statusFilter && dayData?.late > 0

  return (
    <button className={className} onClick={() => onSelect(date)} title={holidayName || undefined}>
      <span className="admin-calendar-daynum">{Number(date.slice(8, 10))}</span>
      {holidayName && <span className="admin-calendar-holiday-dot"></span>}
      {badge}
      {showLateDot && <span className="admin-calendar-late-dot" title={`${dayData.late} late`}></span>}
    </button>
  )
}

// The holiday markers on a calendar grid rely on `title`, which never shows
// on a touch phone (no hover) -- this is the always-visible list underneath,
// for the same month, on both the admin and intern calendars.
      ...
      {entries.map(([date, name]) => {
        ...
        return (
          <div className="holiday-row" key={date}>
            <Calendar size={16} />
            <span className="holiday-date">{parsed.toLocaleDateString([], { month: 'short', day: 'numeric' })}</span>
            <span><strong>{name}</strong><small>{parsed.toLocaleDateString([], { weekday: 'long' })}</small></span>
          </div>
        )
      })}
    </div>
...
function App() {
  ...
  const loadAdminMonthRecords = (month, status, internId) => {
    ...
    const query = new URLSearchParams({ month, status })
  ...
  useEffect(() => {
    if (isAdmin && activeTab === 'admin' && adminStatusFilter) {
  ...
  // Status filter active -> every matching record for the whole month
  // (already filtered server-side). Otherwise -> just the selected day,
  // filtered client-side to the selected intern if any.
  const visibleAdminRecords = adminStatusFilter
    ? adminMonthRecords
    : adminRecords.filter((record) => !adminFilterIntern || record.internId === adminFilterIntern.id)
  ...
  return (
    <div className="app-shell">
      ...
      <main id="overview" className={`tab-content ${activeTab}-tab`}>
        ...
        {activeTab === 'history' && <section className="tab-panel logs-panel"><div className="tab-heading"><p className="eyebrow">ATTENDANCE LOGS</p><h1>My attendance history</h1><p>Clock-ins, clock-outs, late arrivals, and grace-period records.</p></div><article className="activity-card"><div className="admin-calendar"><div className="admin-calendar-header"><button className="icon-button" aria-label="Previous month" onClick={() => shiftHistoryMonth(-1)}><ChevronLeft size={18} /></button><strong>{formatMonthLabel(historyMonth)}</strong><button className="icon-button" aria-label="Next month" onClick={() => shiftHistoryMonth(1)}><ChevronRight size={18} /></button></div><div className="admin-calendar-weekdays"><span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span></div><div className="admin-calendar-grid">{historyWeeks.map((week, weekIndex) => week.map((date, dayIndex) => date ? <div key={date} className={`admin-calendar-day${date === todayDate ? ' is-today' : ''}${historyHolidays[date] ? ' is-holiday' : ''}`} title={historyHolidays[date] || undefined}><span className="admin-calendar-daynum">{Number(date.slice(8, 10))}</span>{historyHolidays[date] && <span className="admin-calendar-holiday-dot"></span>}{historyByDate[date] && <span className={`history-dot ${statusDotClass(historyByDate[date].status)}`} title={historyByDate[date].status}></span>}</div> : <span key={`${weekIndex}-${dayIndex}`} className="admin-calendar-day empty"></span>))}</div></div><HolidayList holidays={historyHolidays} /><div className="history-list">{historyMonthLoading ? <><HistoryRowSkeleton /><HistoryRowSkeleton /><HistoryRowSkeleton /></> : historyMonthRecords.length === 0 ? <p className="drawer-note">No attendance recorded this month.</p> : historyMonthRecords.slice().reverse().map((entry) => <div className="history-row" key={`log-${entry.id}`}><div className="history-date"><strong>{entry.date.split(',')[0]}</strong><span>{entry.date.split(',').slice(1).join(',')}</span></div><div className="history-times"><strong>{entry.clockIn || '—'}</strong><span>{entry.clockOut ? `to ${entry.clockOut}` : 'Still working'}</span></div><span className="mode-tag">{entry.mode}</span><span className={`status-tag ${entry.status === 'On time' ? 'green' : entry.status === 'Excused (MC)' ? 'excused' : 'orange'}`}>{entry.status}</span></div>)}</div></article></section>}

        {activeTab === 'admin' && isAdmin && <section className="tab-panel admin-panel"><div className="tab-heading"><p className="eyebrow">ADMIN</p><h1>Attendance overview</h1><p>Every intern's attendance, and the office QR code.</p></div><article className="activity-card admin-qr-card"><div className="section-heading"><div><p className="eyebrow">OFFICE QR</p><h2>Current code</h2></div><div className="admin-qr-actions"><button className="secondary-action" onClick={downloadQr}><Download size={16} /> Save QR</button><button className="secondary-action" onClick={regenerateQr}><QrCode size={16} /> Regenerate</button></div></div>{qrDataUrl && <img src={qrDataUrl} alt="Office QR code" className="admin-qr-image" />}<p className="drawer-note">Regenerating invalidates the old code immediately -- update the printed or displayed copy at the office right away.</p></article><article className="activity-card"><div className="section-heading"><div><p className="eyebrow">ALL ATTENDANCE</p><h2>{adminStatusFilter ? formatMonthLabel(adminCalendarMonth) : adminDate}</h2></div><div className="admin-filter-bar"><button className={adminStatusFilter === 'Late' ? 'status-filter-button active-late' : 'status-filter-button'} onClick={() => setAdminStatusFilter((previous) => (previous === 'Late' ? null : 'Late'))}>Late</button><button className={adminStatusFilter === 'On time' ? 'status-filter-button active-ontime' : 'status-filter-button'} onClick={() => setAdminStatusFilter((previous) => (previous === 'On time' ? null : 'On time'))}>On time</button>{adminFilterIntern && <button className="filter-chip" onClick={() => setAdminFilterIntern(null)}>{adminFilterIntern.name}<X size={13} /></button>}<a className="secondary-action export-button" href={adminExportUrl} download={adminExportFilename}><Download size={14} /> Export CSV</a></div></div><div className="admin-calendar"><div className="admin-calendar-header"><button className="icon-button" aria-label="Previous month" onClick={() => shiftAdminMonth(-1)}><ChevronLeft size={18} /></button><strong>{formatMonthLabel(adminCalendarMonth)}</strong><button className="icon-button" aria-label="Next month" onClick={() => shiftAdminMonth(1)}><ChevronRight size={18} /></button></div><div className="admin-calendar-weekdays"><span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span></div><div className="admin-calendar-grid">{adminCalendarWeeks.map((week, weekIndex) => week.map((date, dayIndex) => <AdminCalendarDay key={date || `${weekIndex}-${dayIndex}`} date={date} isSelected={date === adminDate} isToday={date === todayDate} dayData={date ? adminCalendarByDate[date] : null} filterIntern={adminFilterIntern} statusFilter={adminStatusFilter} holidayName={date ? adminHolidays[date] : null} onSelect={setAdminDate} />))}</div></div><HolidayList holidays={adminHolidays} /><div className="history-list">{(adminStatusFilter ? adminMonthLoading : adminLoading) ? <p className="drawer-note">Loading...</p> : visibleAdminRecords.length === 0 ? <p className="drawer-note">{adminFilterIntern || adminStatusFilter ? `No${adminStatusFilter ? ` ${adminStatusFilter.toLowerCase()}` : ''} attendance${adminFilterIntern ? ` for ${adminFilterIntern.name}` : ''} ${adminStatusFilter ? 'this month' : 'on this date'}.` : 'No attendance recorded for this date.'}</p> : visibleAdminRecords.map((record) => <div className="history-row" key={record.id}><div className="history-date"><button className="intern-name-button" onClick={() => toggleAdminFilterIntern(record)}>{record.internName}</button><span>{adminStatusFilter ? `${record.date} · ${record.refNumber}` : record.refNumber}</span></div><div className="history-times"><strong>{record.clockIn || '—'}</strong><span>{record.clockOut ? `to ${record.clockOut}` : 'Still working'}</span></div><span className="mode-tag">{record.mode}</span><span className={`status-tag ${record.status === 'On time' ? 'green' : record.status === 'Excused (MC)' ? 'excused' : 'orange'}`}>{record.status}</span></div>)}</div></article></section>}
      </main>
