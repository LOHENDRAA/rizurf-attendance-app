...
import {
  ...
  Home, MapPin, QrCode, ScanLine, Settings,
  ...
  return (
    <div className="app-shell">
      ...
      {profileOpen && <div className="profile-backdrop" onClick={(event) => event.target === event.currentTarget && setProfileOpen(false)}><aside className="profile-drawer"><button className="drawer-close" aria-label="Close profile" onClick={() => setProfileOpen(false)}><X size={19} /></button><div className="drawer-avatar">{initials}</div><p className="eyebrow">INTERN PROFILE</p><h2>{displayName}</h2><div className="credential-list"><div><span>Full legal name</span><strong>{displayName}</strong></div><div><span>Reference</span><strong>{intern?.ref_number || '—'}</strong></div><div><span>Rizurf account</span><strong>{me?.email || '—'}</strong></div><div><span>Arrangement</span><strong>{intern ? `${intern.mode} · ${intern.allowance}` : '—'}</strong></div><div><span>Internship duration</span><strong>{intern ? `${formatDay(intern.internship_start_date)} – ${formatDay(intern.internship_end_date)}` : '—'}</strong></div></div><div className="drawer-setting"><span><Bell size={18} /> Clock in/out reminder</span><button className={notificationsEnabled ? 'toggle is-on' : 'toggle'} aria-pressed={notificationsEnabled} onClick={() => setNotificationsEnabled(!notificationsEnabled)}><i></i></button></div><div className="drawer-setting"><span><ShieldCheck size={18} /> My device</span><strong>{device?.linked ? (device.isYou ? 'Linked to you' : 'Linked elsewhere') : 'Not linked yet'}</strong></div>{device?.linked && device?.isYou && <button className="drawer-setting drawer-action" onClick={releaseDevice}><Settings size={18} /> Unlink this device</button>}<p className="drawer-note">{device?.linked
        ...
        : 'The first time you clock in, this device links to you -- after that, no one else can clock in from it.'}</p><button className="drawer-setting drawer-action"><Settings size={18} /> Account settings</button><p className="drawer-note">Signed in through the Rizurf gateway. Sign out there.</p></aside></div>}
    </div>
  )
}
